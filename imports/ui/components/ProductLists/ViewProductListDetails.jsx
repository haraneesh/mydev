  ...
  <div className="view-product-list-details">
    ...
    {products.map((product) => (
      <Col xs={12} key={product._id}>
        ...
        <FieldGroup
          label="Max Orderable Units"
          value={product.maxUnitsAvailableToOrder}
        />
        <FieldGroup
          label="Total Units Ordered"
          value={product.totQuantityOrdered}
        />
        <hr />
      </Col>
    ))}
...
class ViewProductListDetails extends React.Component {
  ...
  handleRemove(_id) {
    if (
      confirm(
        'Are you sure about deleting this Product List? This is permanent!',
      )
    ) {
      removeProductList.call({ _id }, (error) => {
        if (error) {
          ...
          this.props.navigate('/productLists');
        }
      });
    }
  }
  ...
  handleEdit(_id) {
    this.props.navigate(`/productLists/${_id}/edit`);
  }
      ...
      return (
        <Row>
          <Col>
            ...
            <Button
              size="sm"
              onClick={() => this.handleEdit(productListId)}
              className="btn-secondary me-2"
            >
              Edit
            </Button>
            <Button size="sm" onClick={() => this.handleRemove(productListId)}>
              Delete
            </Button>
          </Col>
        </Row>
  ...
  render() {
    const { productList } = this.props;
    const productListStatus = getProductListStatus(
      productList.activeStartDateTime,
      productList.activeEndDateTime,
    );
    return (
      <div className="ViewProductListDetails px-2">
        <div className="py-2">
          ...
          {this.displayDeleteProductListButton(
            productListStatus,
            productList._id,
          )}
        </div>
        ...
        <Button size="sm" onClick={this.goBack}>
          &larr; Back
        </Button>
      </div>
    );
...
export default withRouter(ViewProductListDetails);
